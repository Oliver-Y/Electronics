...
const uint8_t CMD_MOTOR = 0x01;
const uint8_t CMD_BRAKE = 0x02;
const uint8_t CMD_PWR = 0x03;
const uint8_t CMD_TURN = 0x04;
...
{
//  uint8_t temp = ((m_buffer[0]>>0) & 0b00000111);
  if (motor_buff[0] == CMD_MOTOR) {
    drive_motor(m_buffer[1]);
  }
...
void drive_motor(uint8_t state) {
  if(state){
    /*nrf_gpio_pin_set(4);
    nrf_gpio_pin_set(6);
    nrf_gpio_pin_set(7);
...
//  clear_leds();
  ...
  //Initialize PWM
  //app_pwm_config_t pwm1_cfg = APP_PWM_DEFAULT_CONFIG_2CH(5000L,//Pin 1 for hanel 1 , pin 2 for chanel 2);
